"""atlas_bench_v2.py — the LOCKED evaluator for SPEC 10 under Amendment 1.

Registered as `atlas-wf-v2` (walk-forward, primary) and `atlas-kf-v2` (k-fold, reported
alongside). Supersedes the v1 pair, which is retained as a pilot record together with the
files it was locked against, frozen in legacy/v1_snapshot/.

What changed from v1 (Amendment 1 section 3): every zone-window is scored on the fold's
COMMON usable year set, so the real run and the complete-season target permutation share
their support exactly. No test year is lost -- the earliest walk-forward test year is
2002 -- and at most one early training year is dropped from windows that would have had
it.
...
def main():
    ...
    ap.add_argument("--family", default="atlas/all-windows", choices=P2.FAMILIES)
    ...
    try:
        ...
        ff, folds, parts = P2.run_atlas_v2(
            protocol=a.protocol, n_folds=a.folds,
            arm=("zones-specific" if a.arm == "pooled" else a.arm),
            leak=a.leak, force_k=1 if a.arm == "pooled" else None,
            weight_rule=a.weight)
