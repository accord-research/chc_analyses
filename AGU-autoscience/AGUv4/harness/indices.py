...
def raw_components(sst, months, years, gap, width=WIDTH):
    ...
    Memoised. These depend only on (months, gap, width) and the SST fixture -- never on
    the fold, the arm or the permutation -- but bench.Context.preseason does two xarray
    scalar selects per box per year, so recomputing them inside every fold of every
    replicate cost more than the entire rest of the pipeline. The cache changes no
    number; the scorer-agreement gate re-checks that on every run.
    """
    key = (tuple(months), int(gap), int(width), len(years), int(years[0]), int(years[-1]))
    hit = _COMP_CACHE.get(key)
    if hit is not None:
        return hit
    ctx = bench.Context(sst, months, years, years, years, np.zeros(len(years)))
    out = {b: ctx.feature(b, gap=gap, width=width, years=years) for b in BOXES}
    _COMP_CACHE[key] = out
    return out
